...
class FollowTheGapMethod():
    ...
    def preprocess_lidar2(self, left_range, right_range, ranges):
        """ Preprocess the LiDAR scan array. Expert implementation includes:
            1.Setting each value to the mean over some window
            2.Rejecting high values (eg. > 3m)
        """
        ...
        # we won't use the LiDAR data from directly behind us
        proc_ranges = np.array(ranges[45:-45])  # real lidar
        proc_ranges = np.array(ranges[right_range:left_range]) # simulator lidar
    ...
    def process_lidar2(self, left_range, right_range, ranges):
        """
        Process each LiDAR scan as per the Follow Gap algorithm & publish an AckermannDriveStamped Message
        """
        proc_ranges = self.preprocess_lidar2(left_range, right_range, ranges)
        # Find closest point to LiDAR
        closest = proc_ranges.argmin() # return index of min value in list
        # Eliminate all points inside 'bubble' (set them to zero)
        min_index = closest - self.BUBBLE_RADIUS
    ...
    def inputRawData2(self, range_left, range_right, data):
        if len(data)==0:
            self.speed, self.steering_angle = 0.0, 0.0
        else:
            self.speed, self.steering_angle = self.process_lidar2(range_left, range_right, data)
        return self.speed, self.steering_angle
