...
class TrajectoryPoint():
    ...
    def ros_marker3(self, waypointList, r, g, b):
        ...
        marker.color.b = b
        marker.id =id
        marker.pose.position.x = waypointList[0]
        marker.pose.position.y = waypointList[1]
        marker.pose.position.z = 0
        marker.pose.orientation.w = 1.0
...
class ObstaclePositionEstimation():
    ...
    def waypoint_range(self, waypointList, radians_per_point, waypoint_idx):
        ...
        for w in waypointList_copy:
            ...
            vehicle_width_radian = 2 * np.arcsin(self.width_to_cover / distance)
            vehicle_width_radian = vehicle_width_radian + vehicle_width_radian * distance/100
    ...
    def waypoint_side(self, waypoint_side, waypoint, radian_per_point):
        ...
        # a = np.array([])
        # a = np.append(a, waypoint[0])
        # a = np.append(a, right_y)
        # a = np.append(a, waypoint[0])
        # a = np.append(a, left_y)
        # a= np.reshape(a, int(len(a)/2))
        #
